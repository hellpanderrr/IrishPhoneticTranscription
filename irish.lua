--[[
    Phonetic Transcription Script for Modern Irish (Iteration 37AQ - Epenthesis Precondition Fixes)
    Focus: 
           1. Fix Metathesis output to avoid 'cr''' / 'gr''' intermediate forms.
           2. Overhaul `is_likely_monosyllable_phonetic_revised` for robust nucleus counting.
           3. Refine `parse_phonetic_string_to_units_for_epenthesis` for stress markers and quality assignment.
           4. Re-test epenthesis.
...
local debug_file_path = "irish_debug_37AQ.txt" 
    ...
    while i <= ulen(phon_str) do
        ...
        if matched_unit_phon then
            ...
            elseif umatch(matched_unit_phon, "^[" .. ANY_CONSONANT_PHONETIC_RAW_CHARS_STR .. "]$") and not umatch(matched_unit_phon, "['ʲˠ̪]") then 
                -- For single consonants without explicit marks, infer from context if possible, else default
                -- This part needs more sophisticated context checking for epenthesis,
                -- for now, we'll keep it simple and let the epenthesis rule handle agreement.
                quality = "nonpalatal" -- Default for unmarked single consonants
            ...
            elseif umatch(matched_unit_phon, "^[" .. ALL_SINGLE_PHONETIC_VOWEL_CHARS_STR .. "]$") or umatch(matched_unit_phon, "^[" .. ALL_SINGLE_PHONETIC_VOWEL_CHARS_STR .. "]ː$") then quality = "vowel" 
            end


            table.insert(units, { phon = matched_unit_phon, stress = stress, quality = quality, original_start = i - ulen(stress), original_end = i + ulen(matched_unit_phon) - 1 - ulen(stress) })
            i = i + ulen(matched_unit_phon)
        elseif stress ~= "" then 
            table.insert(units, { phon = stress, stress = "", quality = "stress_mark", original_start = i - 1, original_end = i - 1})
        else 
            table.insert(units, { phon = usub(phon_str,i,i), stress = stress, quality = "unknown_fallback", original_start = i - ulen(stress), original_end = i - ulen(stress) })
            i = i + 1
        end
    end
    return units
    ...
    while i <= #parsed_units do
        if i + 2 <= #parsed_units then
            ...
            if is_vowel_short and is_c1_sonorant_type and is_c2_obstruent_type then
                ...
                debug_print_detailed("EpenthesisAndStrongSonorants", "Checking V-C1-C2: ", unit_v.phon, unit_c1.phon, unit_c2.phon, " | Cluster key: ", cluster_key_for_check, " | C1 Qual: ", c1_quality, " | C2 Qual: ", c2_quality)
            ...
            if ep_vowel_to_insert then
                debug_print_detailed("EpenthesisAndStrongSonorants", "PROCEDURAL Epenthesis Triggered for: ", unit_v.phon, unit_c1.phon, unit_c2.phon, " -> inserting ", ep_vowel_to_insert)
            ...
            while meta_scan_offset <= ulen(phon_word_in_stage3) do
                ...
                if ((c_is_k_type) and n_phon_base == "n") or (c_is_g_type and n_phon_base == "n") then
                     if (meta_scan_offset == 1 and stress_marker == "") or (meta_scan_offset == (1 + ulen(stress_marker)) and stress_marker ~= "") then 
                        ...
                        table.insert(metathesis_phon_parts, stress_marker .. c_phon_base .. (c_is_palatal and "'" or "")) -- Keep original c quality
...
print("\n--- Running Test Set for Iteration 37AP (" .. (#words_to_test_final == #words_to_test_full_37AB and "Full" or "Focused") .. ") ---") 
