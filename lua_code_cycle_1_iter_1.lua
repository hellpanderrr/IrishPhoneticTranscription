...
local debug_file_path = "irish_debug_C1_I1.txt" -- Changed for this iteration
...
-- Phonetic Patterns
local ANY_CONSONANT_PHONETIC_RAW_CHARS_STR = "kgptdfbmnszrlLNRMçjɣŋhwcʃɟɾx" -- Added 'x' for consistency in parsing quality
local ANY_CONSONANT_PHONETIC_PATTERN = "[" .. ANY_CONSONANT_PHONETIC_RAW_CHARS_STR .. "]"
local ANY_SHORT_VOWEL_PHONETIC_CHARS_STR = "aæɔeəiɪuʊʌɛɑ" -- Added 'ɛ' and 'ɑ'
...
local ALL_PHONETIC_NUCLEI_PRIORITY = {
    ...
    "a", "æ", "ɔ", "e", "ə", "i", "ɪ", "u", "ʊ", "ʌ", "ɛ", "ɑ" -- Added ɛ and ɑ
}
...
irishPhonetics.EPENTHESIS_TARGET_CLUSTERS_BROAD = {
    ...
    ["rb"]=true, ["rg"]=true, ["rv"]=true, -- Added rv
    ["lm"]=true, -- added for balbh, gorm related
    ["rl"]=true, ["rn"]=true, ["rm"]=true, -- sonorant+sonorant
}
irishPhonetics.EPENTHESIS_TARGET_CLUSTERS_SLENDER = {
    ["lk"]=true, ["lf"]=true, ["rg"]=true, ["rk"]=true, ["nm"]=true,
    ["l'k'"]=true, ["r'k'"]=true, ["n'm'"]=true, -- ensure palatal clusters are included explicitly
}

-- Rebuilt to prioritize multi-character phonetic symbols and handle single characters correctly.
local PHONETIC_UNITS_PRIORITY_FOR_EPENTHESIS_PARSER = {
    -- Diphthongs
    "iə", "ua", "ai", "ei", "oi", "ui", "ɑu", "ou", "əu", "eiə", "aw", "əi",
    -- Long vowels
    "ɑː", "eː", "iː", "oː", "uː", "ɨː", "æː",
    -- Multi-character consonants (including diacritics and primes)
    "tʲ", "dʲ", "lʲ", "nʲ", "ɾʲ", "fʲ", "vʲ", "bʲ", "pʲ", "mʲ", "Lʲ", "Nʲ", "Rʲ", "Mʲ", "s'", "t̪", "d̪", "n̪", "l̪",
    "ɾˠ", "lˠ", "nˠ", "mˠ", -- Velarized sonorants
    "k'", "g'", "t'", "d'", "p'", "b'", "m'", "n'", "l'", "r'", "s'", "f'", "v'", "L'", "N'", "R'", "M'", -- Generic primes
    -- Single character consonants
    "c", "ɟ", "ʃ", "ç", "j", "k", "g", "t", "d", "p", "b", "m", "n", "l", "r", "s", "f", "v", "L", "N", "R", "M", "x", "ɣ", "ŋ", "h", "w",
    -- Short vowels
    "a", "æ", "ɔ", "e", "ə", "i", "ɪ", "u", "ʊ", "ʌ", "ɛ", "ɑ"
}
...
local function parse_phonetic_string_to_units_for_epenthesis(phon_str)
    ...
    while i <= ulen(phon_str) do
        ...
        -- Try to match the longest possible phonetic unit first
        for _, unit_pattern_str in ipairs(PHONETIC_UNITS_PRIORITY_FOR_EPENTHESIS_PARSER) do
            if usub(phon_str, i, i + ulen(unit_pattern_str) - 1) == unit_pattern_str then
                ...
                break -- Found the longest match, move to next char
            end
        end

        local quality = "unknown"
        if matched_unit_phon then
            -- Assign quality based on phonetic features
            if umatch(matched_unit_phon, "'$") or umatch(matched_unit_phon, "ʲ$") or
               umatch(matched_unit_phon, "^[tcsdʃçjɟ]$") then -- Handle single char palatal sounds like c,ɟ,ʃ
                quality = "palatal"
            elseif umatch(matched_unit_phon, "ˠ$") or umatch(matched_unit_phon, "[̪]$") then
                quality = "nonpalatal"
            elseif umatch(matched_unit_phon, "^[" .. ALL_SINGLE_PHONETIC_VOWEL_CHARS_STR .. "]") then -- Matches all single vowels, long or short
                quality = "vowel"
            elseif umatch(matched_unit_phon, "^[kgptdfbmnsfrRlLNMxhwŋ]$") then -- All other single consonants
                if umatch(matched_unit_phon, "^[jçʃ]$") then quality = "palatal" -- Explicitly palatal
                elseif umatch(matched_unit_phon, "^[vɣŋh]$") then quality = "nonpalatal" -- These are typically nonpalatal, but can have palatal versions
                else quality = "nonpalatal" -- Default for broad consonants
                end
            elseif matched_unit_phon:match(PHONETIC_VOWEL_NUCLEUS_STRING_FOR_CAPTURE) then -- For diphthongs
                if matched_unit_phon:match("^[eiɪɛiə]$") then quality = "palatal"
                else quality = "nonpalatal" end
            end
        ...
        elseif stress ~= "" then
            ...
            -- Fallback for unhandled characters, assign 'unknown_fallback'
            local unknown_char = usub(phon_str,i,i)
            local unknown_quality = "unknown_fallback"
            -- Attempt to categorize as vowel if missed by priority list
...
function irishPhonetics.apply_procedural_epenthesis(phon_word_input, original_ortho_word_for_context, current_ortho_map_for_context)
    ...
    while i <= #parsed_units do
        ...
        if i + 2 <= #parsed_units then
            ...
            local is_c1_sonorant_type = umatch(c1_base_phon, "^[rlnmRMLN]$") -- Include capital sonorants as well
            local c2_base_phon = ugsub(unit_c2.phon, "['ˠʲ̪]", "")

            -- C2 can be an obstruent OR a sonorant if C1 is also a sonorant (for sonorant+sonorant clusters)
            local is_c2_eligible_for_epenthesis = umatch(c2_base_phon, "^[kgptdfbxsxɣ]$") or (is_c1_sonorant_type and umatch(c2_base_phon, "^[rlnmRMLN]$")) -- Added x and ɣ to obstruents

            local c1_quality = unit_c1.quality
            local c2_quality = unit_c2.quality

            -- Infer C1 quality if unknown or broad/neutral when surrounding context is slender
            if is_c1_sonorant_type and (c1_quality == "unknown" or c1_quality == "nonpalatal") then
                if (unit_v.quality == "palatal" or (unit_v.quality == "vowel" and umatch(unit_v.phon, "^[eiɛɪ]$"))) and c2_quality == "palatal" then
                    if unit_c1.phon == c1_base_phon then -- Only add prime if not already marked for quality
                        c1_quality = "palatal"
            ...
            if is_vowel_short and is_c1_sonorant_type and is_c2_eligible_for_epenthesis then
                local cluster_key_for_check = c1_base_phon .. c2_base_phon -- e.g., "lk", "rm", "nm"
                ...
                -- Check against specific target lists based on quality agreement
                ...
                -- Special heuristic for 'nm' where C2 is palatal (ainm) - force C1 to palatal and insert slender epenthetic vowel
                elseif cluster_key_for_check == "nm" and c2_quality == "palatal" then
                    if irishPhonetics.EPENTHESIS_TARGET_CLUSTERS_SLENDER[cluster_key_for_check] then
...
irishPhonetics.rules_stage7_final_cleanup = {
    ...
    { pattern = "k'", replacement = "c" }, { pattern = "g'", replacement = "ɟ" }, -- Added for cleanup
...
function irishPhonetics.apply_vowel_nasalization(phon_word)
    ...
    local NASAL_CONSONANTS_PHONETIC = {
        ...
        ["mʲ"]=true, ["nʲ"]=true -- Assuming N', M' become Nʲ, Mʲ later after stage 7
    }
    ...
    for i, unit_data in ipairs(parsed_units) do
        ...
        if current_quality == "vowel" and not umatch(current_phon, "~") then -- Only nasalize if not already nasalized
            ...
            if i < #parsed_units then
                ...
                if NASAL_CONSONANTS_PHONETIC[next_unit.phon] then
                    ...
                    reason_for_nasalization = "Regressive: Vowel '" .. original_phon_for_debug .. "' before nasal '" .. next_unit.phon .. "'"
                end
            end
            ...
            if not nasalize_this_vowel and i > 1 then
                ...
                if NASAL_CONSONANTS_PHONETIC[prev_unit.phon] then
                    nasalize_this_vowel = true
                    reason_for_nasalization = "Progressive: Vowel '" .. original_phon_for_debug .. "' after nasal '" .. prev_unit.phon .. "'"
                end
            end
            ...
            if nasalize_this_vowel then
                ...
                if umatch(current_phon, "ː$") then -- Ends with length mark
                    current_phon = ugsub(current_phon, "(.)(ː)$", "%1~%2")
                else
                    current_phon = current_phon .. "~"
                end
...
function irishPhonetics.transcribe(orthographic_word)
    ...
    local stages = {
        ...
        {name = "Nasalization", is_procedural_stage = true, func = irishPhonetics.apply_vowel_nasalization},
            ...
            while scan_offset_strong_son <= ulen(phon_word_in_stage5) do
                ...
                if best_rule_this_iter_idx_ss then
                    ...
                    if apply_this_rule_ss then
                        ...
                        elseif type(rule_ss.replacement) == "function" then local call_params_for_rule_func_ss = {full_match_segment_ss}; for _, cap_val_ss in ipairs(actual_captures_for_rule_func_current_rule_ss) do table.insert(call_params_for_rule_func_ss, cap_val_ss) end; replacement_text_ss = rule_ss.replacement(table.unpack(call_params_for_rule_func_ss)) end
...
-- Full test set provided in the prompt for 37AB
...
if RUN_FULL_TEST_SET then
    local temp_full_set = {}
    local seen_in_full = {}
    for _,w in ipairs(words_to_test_full_37AB) do if not seen_in_full[w] then table.insert(temp_full_set, w); seen_in_full[w]=true end end
...
print("\n--- Running Test Set for Iteration C1.I1 (Nasalization Focus + Critical Fixes) ---")
